...
export default function ReceivingScreen({ thought, bg, isLight, onComplete }: Props) {
  const c = tk(isLight)
  ...
  useEffect(() => {
    ...
    ids.push(setTimeout(() => setCaptionIdx(0), 900))
    ids.push(setTimeout(() => setCaptionIdx(1), 2100))
    ids.push(setTimeout(() => {
      setCaptionOut(true)
      setThoughtOut(true)
    }, 3300))
    ids.push(setTimeout(onComplete, 3900))
    ...
    <div className="fixed inset-0 z-50 overflow-hidden" style={bgStyle(bg)}>
      ...
      {Array.from({ length: RIPPLE_COUNT }, (_, i) => (
        <div
          ...
          style={{
            ...
            width: '60vmax',
            height: '60vmax',
            border: `1px solid ${ring(String(0.35 - i * 0.05))}`,
            animation: `ripple 2.6s cubic-bezier(0.2,0.6,0.4,1) ${i * 480}ms both`,
            pointerEvents: 'none',
          }}
        />
      ))}
      ...
      {[0.12, 0.07].map((opacity, i) => (
        <div
          ...
          style={{
            ...
            fontStyle: 'italic',
            lineHeight: 1.25,
            color: isLight ? `rgba(0,0,0,${opacity})` : `rgba(255,255,255,${opacity})`,
            animation: `echo-drift 3.2s ease-out ${i * 200 + 200}ms both`,
            pointerEvents: 'none',
            whiteSpace: 'pre-wrap',
          }}
        >
      ...
      <div
        ...
        style={{
          ...
          lineHeight: 1.25,
          color: c.text,
          animation: thoughtOut
            ? 'thought-out 0.65s ease-in forwards'
            : 'thought-in 0.7s cubic-bezier(0.2,0.8,0.4,1) forwards',
          whiteSpace: 'pre-wrap',
        }}
      >
        “{thought}”
        ...
        {captionIdx >= 0 && CAPTIONS[captionIdx] && (
          <span
            ...
            style={{
              ...
              animation: captionOut
                ? 'caption-out 0.6s ease-in forwards'
                : 'caption-in 0.5s ease-out forwards',
            }}
          >
            {CAPTIONS[captionIdx]}
          </span>
