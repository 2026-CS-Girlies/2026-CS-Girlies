...
export default function ChatPanel({ isLight, messages, openingThought, input, onInputChange, onSend, onKeyDown, isLoading, isComplete, error, placeholder = 'Type your response...', completePlaceholder = 'Ready for your reflection', bottomRef, inputRef, children }: Props) {
  const c = tk(isLight)
    ...
    <div className="relative z-10 flex-1 flex flex-col mx-3 md:mx-6 mb-3 md:mb-5 rounded-[24px] overflow-hidden" style={{ background: c.panelBg, border: `1px solid ${c.border}`, backdropFilter: 'blur(28px) saturate(1.4)', boxShadow: c.panelShadow }}>
      ...
      <div className="px-4 md:px-8 pt-3 pb-2 flex-none" style={{ borderTop: `1px solid ${c.divider}` }}>
        <div className="flex items-center gap-3 rounded-2xl px-4 py-2.5 md:py-3" style={{ background: c.inputBg, border: `1px solid ${c.inputBorder}` }}>
          ...
          <button onClick={onSend} disabled={isLoading || isComplete} className="w-8 h-8 rounded-full flex items-center justify-center disabled:opacity-40" style={{ background: c.sendBg, border: `1px solid ${c.sendBorder}` }} aria-label="Send message">
